...
function Controller(HTMLgamepad) {
  ...
  function checkInputs() {
    ...
    if (
      "LEFT_ANALOG_STICK_HOR" in analogMap &&
      "LEFT_ANALOG_STICK_VERT" in analogMap
    ) {
      processAnalogStick.call(this, "LEFT_ANALOG_STICK", {
        x: getNormalizedAnalogInput(
          gamepad.axes[analogMap.LEFT_ANALOG_STICK_HOR]
        ),
        y: getNormalizedAnalogInput(
          gamepad.axes[analogMap.LEFT_ANALOG_STICK_VERT]
        ),
      });
    }
    if (
      "RIGHT_ANALOG_STICK_HOR" in analogMap &&
      "RIGHT_ANALOG_STICK_VERT" in analogMap
    ) {
      processAnalogStick.call(this, "RIGHT_ANALOG_STICK", {
        x: getNormalizedAnalogInput(
          gamepad.axes[analogMap.RIGHT_ANALOG_STICK_HOR]
        ),
        y: getNormalizedAnalogInput(
          gamepad.axes[analogMap.RIGHT_ANALOG_STICK_VERT]
        ),
      });
    }
  ...
  function loopThroughButtons() {
    ...
    for (var name in inputs) {
      ...
      var value =
        _typeof(gamepad[section][_index3]) === "object"
          ? gamepad[section][_index3].value
          : gamepad[section][_index3];
  ...
  function processButton(name, value) {
    ...
    var inputName =
      name.startsWith("L_") || name.startsWith("R_") ? name.substring(2) : name;
    var button = state.buttons[name] || state.axes[name] || state.misc[name];
    if (
      eventQueue[inputName] &&
      eventQueue[inputName].name ===
        this.constructor.events.getName("button", "during")
    ) {
      return;
    }
    if (typeof button.getOption("method") === "function") {
      value = button
        .getOption("method")
        .call(this, this.constructor.gamepads[this.index], button);
    }
    ...
    if (pressed && !previouslyPressed) {
      queueEvent(
        inputName,
        this.constructor.events.getName("button", "start"),
        copy
      );
      ...
      queueEvent(
        inputName,
        this.constructor.events.getName("button", "end"),
        copy
      );
    }
  }
  function processAnalogStick(name, input) {
    ...
    if (
      ...
      eventQueue[name].name ===
        this.constructor.events.getName("analog", "during")
    ) {
      return;
    }
    var analogStick =
      state.axes[name] || state.buttons[name] || state.misc[name];
    ...
    var changed =
      analogStick.position.x !== previousValues.x ||
      analogStick.position.y !== previousValues.y;
    var copy = analogStick.copy();
    if (pressed && !previouslyPressed) {
      queueEvent(
        name + "start",
        this.constructor.events.getName("analog", "start"),
        copy
      );
    } else if (pressed && previouslyPressed) {
      queueEvent(
      ...
      queueEvent(
        name + "end",
        this.constructor.events.getName("analog", "end"),
        copy
      );
    }
    if (changed) {
      queueEvent(
        name + "change",
        this.constructor.events.getName("analog", "change"),
        copy
      );
    }
  }
  function processAnalogAsDpad(stick) {
    ...
    var analogStick =
      stick === "left" ? "LEFT_ANALOG_STICK" : "RIGHT_ANALOG_STICK";
    ...
    try {
      for (
        var _iterator = names[Symbol.iterator](), _step;
        ...
        switch (name) {
          case "DPAD_UP":
            ...
            isPastThreshold =
              inputAxis <= this.settings.analogStickDpadThreshold * -1;
            break;
          ...
          case "DPAD_DOWN":
            ...
            otherAxis = state.axes[analogStick].position.x;
            isPastThreshold =
              inputAxis >= this.settings.analogStickDpadThreshold;
            break;
        ...
        var value =
          isPastThreshold && Math.abs(inputAxis) > Math.abs(otherAxis) ? 1 : 0;
  ...
  function calculateAnalogValues(values) {
    ...
    if (values.x !== 0) {
      output.x = getLinearConversion(
        Math.abs(values.x),
        this.settings.analogStickDeadzone.min,
        this.settings.analogStickDeadzone.max,
        0,
        1
      );
    ...
    return this.settings.mapAnalogToShape === "square"
      ? getCoordsMappedToSquare(output)
      : output;
  }
  ...
  function getCoordsMappedToSquare(coords) {
    if (coords.x === 0 || coords.y === 0) {
      ...
      var newVals = getShortenedLineDistance(
        abs,
        getLinearConversion(oldLength, 0, 1, 0, max)
      );
      return {
  ...
  Object.defineProperty(this, "name", {
    get: function get() {
      ...
      try {
        for (
          var _iterator2 = Controller.layouts.regex[Symbol.iterator](), _step2;
...
Math.hypot =
  ...
  function () {
    ...
    try {
      for (
        var _iterator3 = arguments[Symbol.iterator](), _step3;
...
var GC_Settings = function GC_Settings(isGlobalSettingList) {
  ...
  this.register = function (setting) {
    if (!isGlobalSettingList) {
      console.warn(
        "You can only register settings globally:\nController.globalSettings.register()"
      );
      return false;
    }
    ...
    Object.defineProperty(this.constructor.prototype, setting.name, {
      ...
      set: function set(value) {
        if (value === undefined || (value === "default" && this.global)) {
          ...
          if (_typeof(setting.defaultValue) === "object") {
            localSettings[setting.name] = Object.assign(
              {},
  ...
  switch (arguments.length) {
    case 1:
      if (_typeof(arguments[0]) === "object") {
        ...
        console.warn(
          'GC_Settings.update(settings) expects "settings" to be an object of key/value pairs.'
        );
      }
      break;
    ...
    case 2:
      if (typeof arguments[0] === "string" && arguments[0] in this) {
        ...
        console.warn(
          'GC_Settings.update(settingName, value) expects "settingname" to be the name of a setting.'
        );
      }
      break;
  ...
  new GC_Setting(
    ...
    function (value) {
      if (
        ...
        console.warn(
          'Value must be an object containing either/both "min" and "max" values'
        );
        return null;
      }
      var output = {};
...
Controller.globalSettings.register(
  new GC_Setting("analogStickDpadThreshold", 0.7, function (value) {
...
Controller.globalSettings.register(
  new GC_Setting("buttonThreshold", 0.95, function (value) {
    if (typeof value === "number" && value >= 0 && value <= 1) {
      return value;
    } else {
      console.warn("angry");
...
Controller.globalSettings.register(
  new GC_Setting("mapAnalogToShape", "none", function (value) {
    if (value === "none") {
      return "none";
...
Controller.globalSettings.register(
  new GC_Setting("useAnalogAsDpad", "none", function (value) {
