import { GC_Events } from "./lib/GC_Events";
import { GC_Layouts, StandardLayout } from "./lib/GC_Layouts";
import { GC_Settings, GC_Setting } from "./lib/GC_Settings.js";
import {
  useAnalogAsDpad,
  analogStickDeadzone,
...
function Controller(HTMLgamepad) {
  ...
  function loopThroughButtons() {
    ...
    for (let name in inputs) {
      ...
      const value =
        typeof gamepad[section][index] === "object"
          ? gamepad[section][index].value
          : gamepad[section][index];
  ...
  function getCoordsMappedToSquare(coords) {
    if (coords.x === 0 || coords.y === 0) {
      ...
      const newVals = getShortenedLineDistance(
        abs,
...
Controller.globalSettings.register(
  new GC_Setting("analogStickDeadzone", { min: 0, max: 1 }, analogStickDeadzone)
);
Controller.globalSettings.register(
  new GC_Setting("analogStickDpadThreshold", 0.7, analogStickDpadThreshold)
);
Controller.globalSettings.register(
  new GC_Setting("buttonThreshold", 0.95, buttonThreshold)
);
Controller.globalSettings.register(
